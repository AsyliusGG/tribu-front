...
import { getallEventos, getallSector } from "../../api/api";
  ...
  useEffect(() => {
    const fetchEventos = async () => {
      try {
        ...
        const eventosResponse = await getallEventos();
        const sectoresResponse = await getallSector();
        const eventosActivos = eventosResponse.data.filter(
          (evento) => !evento.disabled
        );
            ...
            >
              ...
              <CardFooter className="flex justify-end">
                <Button
                  variant="gradient"
                  color="blue"
                  onClick={() => navigate(`/Eventos/VerEvento/${post.id}`)}
                >
                  Ver Detalles
                </Button>
              </CardFooter>
